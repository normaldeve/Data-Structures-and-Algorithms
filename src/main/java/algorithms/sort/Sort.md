...
퀵 정렬은 **피벗(Pivot)**을 기준으로 데이터를 나누어 정렬하는 알고리즘입니다.
피벗을 기준으로 배열을 두 부분으로 분할한 뒤, 각각에 대해 재귀적으로 정렬을 수행합니다.

### 알고리즘 단계
1. **피벗**을 설정합니다 (보통 배열의 마지막 원소를 선택).
2. 배열을 순회하며 피벗보다 작은 값은 왼쪽, 큰 값은 오른쪽으로 배치합니다.
3. 피벗을 중간 위치에 배치하여 두 개의 부분 배열로 나눕니다.
4. 각 부분 배열에 대해 재귀적으로 퀵 정렬을 수행합니다.

### 시간 복잡도
- **최악의 경우 (O(n^2))**: 배열이 이미 정렬되어 있거나, 피벗이 최솟값 또는 최댓값으로 선택되는 경우.
- **평균적 경우 (O(n log n))**: 대부분의 경우.

### 피벗 선택 방법
- 배열의 첫 번째, 마지막, 또는 중간값 선택.
- 배열 내 임의의 3개 값을 선택하여 그 중 중앙값을 피벗으로 설정.
...
public class QuickSort {
    ...
    public static int partition(int low, int high) { // Pivot의 최종 위치를 반환
        int pivot = arr[high];
        int i = low - 1;
